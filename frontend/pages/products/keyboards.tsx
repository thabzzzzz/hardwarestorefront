...
export default function KeyboardsListing(): JSX.Element {
  ...
  const manufacturers = useMemo(() => ["Epomaker","Intendvision","TROPRO","MageGee","ZORNHER","Logitech","HyperX","Redragon","RAZER","MSI","Corsair","BFKK","GPERHUAN"], [])
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true)
      try {
        let url = `${API_BASE}/api/keyboards?per_page=${perPage}&page=${page}`
        ...
        console.error('fetch keyboards failed', e)
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <div className={styles.page}>
      <Head>
        <title>Keyboards - WiredWorkshop</title>
      </Head>
      <Header />
      <main className={`${styles.main} ${pageStyles.main}`}>
        <nav className={pageStyles.breadcrumb}>Home / Hardware / Peripherals / Keyboards</nav>
        <h1 className={pageStyles.title}>Keyboards</h1>
            ...
            <div className={pageStyles.grid}>
              {loading
                ? Array.from({ length: 12 }, (_, i) => (
                    <div className={pageStyles.skelCard} key={`keyboard-skel-${i}`}>

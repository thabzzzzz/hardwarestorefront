...
export default function CaseFansListing(): JSX.Element {
  ...
  const manufacturers = useMemo(() => [
    "Bailink","be quiet!","Corn Electronics","Fractal Design","GPERHUAN",
    "JONSBO","Lian Li","NZXT","PANO-MOUNTS","PCCOOLER","Rosewill",
    "Super Flower","UTLGAMENG","Valkyrie"
  ], [])
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true)
      try {
        let url = `${API_BASE}/api/case-fans?per_page=${perPage}&page=${page}`
        ...
        console.error('fetch case-fans failed', e)
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <div className={styles.page}>
      ...
      <main className={`${styles.main} ${pageStyles.main}`}>
        ...
        <h1 className={pageStyles.title}>Fans & Coolers</h1>
            ...
            <div className={pageStyles.grid}>
              {loading
                ? Array.from({ length: 12 }, (_, i) => (
                    <div className={pageStyles.skelCard} key={`casefan-skel-${i}`}>

...
export default function PsuListing(): JSX.Element {
  const [items, setItems] = useState<PsuItem[]>([])
  ...
  const manufacturers = useMemo(() => [
    "ASRock","MSI","Raidmax","Thermaltake","Super Flower","Corsair","Vetroo","segotep","COUGAR","SAMA"
  ], [])
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true)
      try {
        let url = `${API_BASE}/api/psus?per_page=${perPage}&page=${page}`
        ...
        console.error('fetch psus failed', e)
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <div className={styles.page}>
      <Head>
        <title>Power Supplies (PSU) - WiredWorkshop</title>
      </Head>
      <Header />
      <main className={`${styles.main} ${pageStyles.main}`}>
        <nav className={pageStyles.breadcrumb}>Home / Hardware / Computer Accessories / PSUs</nav>
        <h1 className={pageStyles.title}>PSUs / Power Supplies</h1>
            ...
            <div className={pageStyles.grid}>
              {loading
                ? Array.from({ length: 12 }, (_, i) => (
                    <div className={pageStyles.skelCard} key={`psu-skel-${i}`}>

...
export default function MonitorsListing(): JSX.Element {
  ...
  const manufacturers = useMemo(() => ["ASRock Phantom Gaming","Sealan","SAMSUNG","ASUS","MSI","CRUA","Uperfect","GIGABYTE","Z-EDGE","Acer America"], [])
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true)
      try {
        let url = `${API_BASE}/api/monitors?per_page=${perPage}&page=${page}`
  ...
  return (
    <div className={styles.page}>
      <Head>
        <title>Monitors & Displays - WiredWorkshop</title>
      </Head>
      <Header />
      <main className={`${styles.main} ${pageStyles.main}`}>
        <nav className={pageStyles.breadcrumb}>Home / Hardware / Peripherals / Monitors</nav>
        <h1 className={pageStyles.title}>Monitors / Screens</h1>
            ...
            <div className={pageStyles.grid}>
              {loading
                ? Array.from({ length: 12 }, (_, i) => (
                    <div className={pageStyles.skelCard} key={`monitor-skel-${i}`}>

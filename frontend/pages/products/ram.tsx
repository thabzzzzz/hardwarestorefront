...
import pageStyles from './gpus.module.css' // Reusing GPU styles for now as they are generic product listing styles
...
export default function RamListing(): JSX.Element {
  ...
  const manufacturers = useMemo(() => ["Patriot Memory","G.SKILL","Team Group","Timetec","Corsair","Crucial","A-Tech","V-color"], [])
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true)
      try {
        let url = `${API_BASE}/api/ram?per_page=${perPage}&page=${page}`
        ...
        console.error('fetch ram failed', e)
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <div className={styles.page}>
      <Head>
        <title>Desktop Memory (RAM) - WiredWorkshop</title>
      </Head>
      <Header />
      <main className={`${styles.main} ${pageStyles.main}`}>
        <nav className={pageStyles.breadcrumb}>Home / Hardware / Memory</nav>
        <h1 className={pageStyles.title}>Memory / RAM</h1>
            ...
            <div className={pageStyles.grid}>
              {loading
                ? Array.from({ length: 12 }, (_, i) => (
                    <div className={pageStyles.skelCard} key={`ram-skel-${i}`}>

...
export default function CasesListing(): JSX.Element {
  ...
  const manufacturers = useMemo(() => [
    "Antec","MXZ","JONSPLUS","Raidmax","DIYPC","PCCOOLER","Montech",
    "Lian Li","Corsair","Zalman","Phanteks","JONSBO","Fractal Design","COUGAR"
  ], [])
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true)
      try {
        let url = `${API_BASE}/api/cases?per_page=${perPage}&page=${page}`
  ...
  return (
    <div className={styles.page}>
      ...
      <main className={`${styles.main} ${pageStyles.main}`}>
        ...
        <h1 className={pageStyles.title}>Computer Cases</h1>
            ...
            <div className={pageStyles.grid}>
              {loading
                ? Array.from({ length: 12 }, (_, i) => (
                    <div className={pageStyles.skelCard} key={`case-skel-${i}`}>

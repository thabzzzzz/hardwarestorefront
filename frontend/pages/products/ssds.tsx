...
export default function SsdListing(): JSX.Element {
  const [items, setItems] = useState<SsdItem[]>([])
  ...
  const manufacturers = useMemo(() => ["Patriot Memory","Team Group","SAMSUNG","Crucial","KingSpec","SanDisk","SK hynix"], [])
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true)
      try {
        let url = `${API_BASE}/api/ssds?per_page=${perPage}&page=${page}`
        ...
        console.error('fetch ssds failed', e)
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <div className={styles.page}>
      ...
      <main className={`${styles.main} ${pageStyles.main}`}>
        ...
        <h1 className={pageStyles.title}>Solid State Drives / SSDs</h1>
            ...
            <div className={pageStyles.grid}>
              {loading
                ? Array.from({ length: 12 }, (_, i) => (
                    <div className={pageStyles.skelCard} key={`ssd-skel-${i}`}>

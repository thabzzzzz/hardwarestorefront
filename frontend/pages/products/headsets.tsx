...
import pageStyles from './gpus.module.css' // Reusing GPU styles
...
export default function HeadsetsListing(): JSX.Element {
  const [items, setItems] = useState<HeadsetItem[]>([])
  ...
  const manufacturers = useMemo(() => [
    "Corsair","FiiO","Fractal Design","HP","HyperX",
    "Jabra Enterprise Products","Logitech","RAZER",
    "SteelSeries","Turtle Beach/Voyetra"
  ], [])
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true)
      try {
        let url = `${API_BASE}/api/headsets?per_page=${perPage}&page=${page}`
        ...
        console.error('fetch headsets failed', e)
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <div className={styles.page}>
      <Head>
        <title>Headsets & Audio - WiredWorkshop</title>
      </Head>
      <Header />
      <main className={`${styles.main} ${pageStyles.main}`}>
        <nav className={pageStyles.breadcrumb}>Home / Hardware / Peripherals / Headsets</nav>
        <h1 className={pageStyles.title}>Headsets & Audio</h1>
            ...
            <div className={pageStyles.grid}>
              {loading
                ? Array.from({ length: 12 }, (_, i) => (
                    <div className={pageStyles.skelCard} key={`headset-skel-${i}`}>
                ...
                : (items.length === 0 ? <div className={pageStyles.noResults}>No products found.</div> : sortedProducts.map(it => (
                    <ProductCard
                      key={it.variant_id}
                      name={(it as any).name}
                      title={it.title}

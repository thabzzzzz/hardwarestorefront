...
import pageStyles from './gpus.module.css' // Reusing GPU styles for consistency
...
export default function MotherboardsListing(): JSX.Element {
  const [items, setItems] = useState<MotherboardItem[]>([])
  ...
  // Available manufacturers based on DB query
  const manufacturers = useMemo(() => ["ASRock","ASUS","GIGABYTE","MSI"], [])
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true)
      try {
        let url = `${API_BASE}/api/motherboards?per_page=${perPage}&page=${page}`
        ...
        console.error('fetch motherboards failed', e)
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <div className={styles.page}>
      ...
      <main className={`${styles.main} ${pageStyles.main}`}>
        ...
        <h1 className={pageStyles.title}>Motherboards</h1>
            ...
            <div className={pageStyles.grid}>
              {loading
                ? Array.from({ length: 12 }, (_, i) => (
                    <div className={pageStyles.skelCard} key={`mobo-skel-${i}`}>

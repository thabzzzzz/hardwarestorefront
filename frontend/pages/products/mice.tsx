...
export default function MiceListing(): JSX.Element {
  const [items, setItems] = useState<MouseItem[]>([])
  ...
  const manufacturers = useMemo(() => ["HP","ASUS","Logitech","MSI","RAZER","Redragon","Corsair","Fantech","Corn Electronics","GravaStar"], [])
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true)
      try {
        let url = `${API_BASE}/api/mice?per_page=${perPage}&page=${page}`
  ...
  return (
    <div className={styles.page}>
      <Head>
        <title>Mice - WiredWorkshop</title>
      </Head>
      <Header />
      <main className={`${styles.main} ${pageStyles.main}`}>
        <nav className={pageStyles.breadcrumb}>Home / Hardware / Peripherals / Mice & Controllers</nav>
        <h1 className={pageStyles.title}>Mice & Controllers</h1>
            ...
            <div className={pageStyles.grid}>
              {loading
                ? Array.from({ length: 12 }, (_, i) => (
                    <div className={pageStyles.skelCard} key={`mouse-skel-${i}`}>

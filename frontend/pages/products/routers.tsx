...
export default function RoutersListing(): JSX.Element {
  ...
  const manufacturers = useMemo(() => ["ASUS","Gigastone Corporation","Netgear Inc.","TP-Link","Ubiquiti Networks","Wavlink"], [])
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true)
      try {
        let url = `${API_BASE}/api/routers?per_page=${perPage}&page=${page}`
        ...
        console.error('fetch routers failed', e)
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <div className={styles.page}>
      <Head>
        <title>Networking & Routers - WiredWorkshop</title>
      </Head>
      <Header />
      <main className={`${styles.main} ${pageStyles.main}`}>
        <nav className={pageStyles.breadcrumb}>Home / Hardware / Networking / Routers</nav>
        <h1 className={pageStyles.title}>Routers</h1>
            ...
            <div className={pageStyles.grid}>
              {loading
                ? Array.from({ length: 12 }, (_, i) => (
                    <div className={pageStyles.skelCard} key={`router-skel-${i}`}>

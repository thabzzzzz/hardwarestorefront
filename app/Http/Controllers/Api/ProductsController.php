    ...
    public function index(Request $request)
    {
        ...
        $data = $page->through(function ($variant) {
            ...
            return [
                ...
                'sku' => $variant->sku,
                'title' => $variant->title,
                // CPU spec fields (nullable) - prefer normalized specs if product columns are empty
                'cores' => $variant->product->cores ?: ($variant->normalized_specs['cores'] ?? null),
                'boost_clock' => $variant->product->boost_clock,
                'microarchitecture' => $variant->product->microarchitecture,
                'socket' => $variant->product->socket ?: ($variant->normalized_specs['socket'] ?? null),
                // GPU spec fields (nullable)
                'vram_gb' => $variant->vram_gb_int,
                'vram_type' => $variant->vram_type,
                'bus_width' => $variant->bus_width_int ? ($variant->bus_width_int . '-Bit') : null,
                'boost_clock_mhz' => $variant->boost_clock_mhz,
                'tdp_watts' => $variant->tdp_watts_int,
                'length_mm' => $variant->normalized_specs['length_mm'] ?? null,
                'recommended_psu' => $variant->normalized_specs['recommended_psu'] ?? null,

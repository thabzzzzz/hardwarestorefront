...
function looksLikeBrand(candidate) {
  ...
  if (!c || c === '-' ) return false;
  // skip obvious non-brand tokens
  const junkPatterns = [
    /\d/, /mm/, /gb/, /tb/, /rpm/, /mhz/, /hz/, /w$/, /\$/, /%/, /\bnewegg\b/, /series/, /kit/, /rgb/, /black/, /white/, /yes|no/, /\bpcs?\b/, /x\d+/, /\d+gb/, /\bmm\b/, /\bcm\b/, /\binches?\b/, /\bmm\b/, /\bitem\b/, /\bpack\b/, /\bcapacity\b/, /\bcompatibility\b/, /\bmodel\b/, /\bprice\b/, /\bweight\b/, /\bsize\b/, /\bcolor\b/, /\btype\b/, /\bform factor\b/, /\binterface\b/, /\bprotocol\b/, /\bdrive\b/, /\bthreadripper\b/, /\bcore\b/, /\bchipset\b/, /\bcontroller\b/, /\bmemory\b/, /\bddr\b/, /\bpcie\b/, /\bhdmi\b/, /\blow profile\b/, /\bprofile\b/, /\bno\b/ 
  ];
  for (const re of junkPatterns) if (re.test(c)) return false;
  // short tokens like 'hd' are probably not brands
  if (c.length <= 2) return false;
  // allow common punctuation in names
  return true;
}
...
const topRows = topLines.slice(1).map(line => {
  // product_type,candidate,occurrences,sources (candidate may contain commas but file was generated simply)
...
// Merge header + existingRows + newRows
const out = [];
out.push('product_type,candidate,canonical_name,notes');
// keep existing rows first (preserve any manual edits)
